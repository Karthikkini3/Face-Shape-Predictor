...
ap.add_argument("-i","--input",required=True,help="path to iBug 300-W data spit XML file")
...
#utilise regular expressions to determine if there is a 'part'
#element on any item given line
PART=re.compile("part name='[0-9]+'")

#load the contents of the original XML file and open the output file
#for writing
print("[INFO] parsing data spilt XML file")
...
for row in rows:
    ...
    #otherwise, there is annotation information that we must process
        ...
        #parse out the name of the attribute form the row
        attr="name='"
        i=row.find(attr)
        j=row.find("'",i+len(attr)+1)
        name=int(row[i+len(attr):j])

        #if the facial landmark name exists within the range of out
