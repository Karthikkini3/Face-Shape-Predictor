...
#define the depth of each regression tree -- there will bw a total of 
#2^tree_depth tree leaves in each tree; small values of tree_depth will be
#faster but less acccurate while larger vales will generate trees that are
#deeper, more accurate, but will run far slower when making predictions
options.tree_depth=4

#regularization parameter in the range of [0,1] that is used to help
...
#to zero will help our model generalize but will require us to have 
#training data in the order of 100s of data points
options.nu=0.1

# the number of cascades used to train the shape predictor -- this parameter
# has a dramantic impact on both the accuracy and output size of your model; 
# the more cascades you have, the more accurate your model can potentially be,
# but also the larger the output size
...
# use larger values if speed is not a problem, otherwise smaller values for 
